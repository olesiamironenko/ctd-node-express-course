...
// - Create a new array with only each person's last name
const lastNamesEach = names.map((name) => name.split(" ").pop());
console.log("All last names", lastNamesEach);

// - Filter names that don't match the format "<first> <last>"
const rightFormat = /^[A-Za-z]+ [A-Za-z]+$/;
const validNames = names.filter((name) => rightFormat.test(name));
console.log("Names in right format: ", validNames);

// - Create a new array where everyone's name is converted to "Title Case"
const titledCaseNames = names.map((name) =>
  name
    .split(/ +/)
    .map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
    .join(" ")
);
console.log("Titled Case Names:", titledCaseNames);
...
const validPropperNames = (names) => {
  const validNames = names
    ...
    .filter(
      (name) => rightFormat.test(name) && !name.split(" ").pop().endsWith("z")
    );
  ...
  return normalizedNames(validNames);
};

console.log("valid Propper Names: ", validPropperNames(names));

const messages = (array) =>
  array.map(
    (name) => `Hello ${name}! Please, sign up to get access to all features`
  );

console.log("messages: ", messages(validPropperNames(names)));

// validNames = filter
// titledCaseNames
// lastNamesEach ends !z
// message
